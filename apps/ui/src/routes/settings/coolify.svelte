...
<script lang="ts">
	...
	async function checkForUpdate() {
		try {
			loading.checkUpdate = true;
			const response = await get(`/update`);
			isUpdateAvailable = response.isUpdateAvailable;
			latestVersion = response.latestVersion;
			if (!isUpdateAvailable) {
				addToast({ message: 'You are already on the latest version.', type: 'success' });
			}
		} catch (error) {
			return errorNotification(error);
		} finally {
			loading.checkUpdate = false;
		}
	}

	async function updateCoolify() {
		const sure = confirm('Are you sure you want to update Coolify? It will pull latest code from GitHub and rebuild.');
		if (sure) {
			try {
				loading.update = true;
				addToast({ message: 'Update started... this may take a few minutes.', type: 'success' });
				await post(`/update`, { type: 'update' });
				addToast({ message: 'Update completed! Waiting for restart...', type: 'success' });
				let reachable = false;
				let tries = 0;
				do {
				...
				addToast({ message: 'New version reachable. Reloading...', type: 'success' });
			...
			<div class="grid grid-flow-row gap-2 px-4 pr-5">
				...
				<div class="grid grid-cols-2 items-center pb-6">
					<div>
						...
						<Explainer
							position="dropdown-bottom"
							explanation="Pull latest code from GitHub and rebuild. Current version: {$appSession.version}"
						/>
					</div>
					<div class="flex flex-row space-x-2">
						<button
							class="btn btn-sm"
							class:loading={loading.checkUpdate}
							disabled={loading.checkUpdate}
							on:click|preventDefault={checkForUpdate}
						>
							{loading.checkUpdate ? 'Checking...' : 'Check for Update'}
						</button>
						{#if isUpdateAvailable}
							<button
								...
								on:click|preventDefault={updateCoolify}
							>
								{loading.update ? 'Updating...' : `Update Now (${latestVersion})`}
							</button>
						{/if}
					</div>
				</div>
