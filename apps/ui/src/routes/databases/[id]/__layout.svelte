...
{#if id !== 'new'}
	<nav class="header lg:flex-row flex-col-reverse">
		...
		<a
			id="dbterminal"
			href={$status.database.isRunning ? `/databases/${id}/terminal` : null}
			class="hover:text-green-400 rounded"
			class:text-green-400={$page.url.pathname === `/databases/${id}/terminal`}
			class:bg-coolgray-500={$page.url.pathname === `/databases/${id}/terminal`}
		>
			<button disabled={!$status.database.isRunning} class="icons bg-transparent text-sm">
			...
			></a
		>
		<Tooltip triggeredBy="#dbterminal">{'Terminal'}</Tooltip>
			{#if forceDelete}
				<button
